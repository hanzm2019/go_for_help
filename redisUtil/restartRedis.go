...
func makeConn(addr, pwd string) (conn *redisConn, err error) {
	...
	c, err := redis.Dial("tcp", addr,
		...
		redis.DialReadTimeout(dataTimeout),
		redis.DialWriteTimeout(dataTimeout))
	if err != nil {
		logger.Error("makeConn|DialTimeout|%v", err)
		return
	}
	if pwd != "" {
		if _, err = c.Do("AUTH", pwd); err != nil {
			logger.Error("makeConn|auth|%v", err)
			c.Close()
			return
		}
	}
	if _, err = c.Do("get", "__test"); err != nil {
		logger.Error("makeConn|get|%v", err)
		c.Close()
		return
	}
...
func (pool *Pool) checkEvent() {
	...
	for {
		...
		case newMaster := <-pool.newMasterCh:
			logger.Info("checkEvent|newMasterCh|%v", newMaster)
			var found = false
			for i := 0; i < len(pool.addrs); i++ {
				if pool.addrs[i] == newMaster {
					//logger.Info("checkNewMaster|found")
			...
			if !found {
				logger.Error("checkEvent|newMasterCh|not_found|%v", newMaster)
			}
		case newAddr := <-pool.addrCh:
			//logger.Info("checkEvent|addrCh|%v", newAddr)
			pool.cleanConn()
			pool.poolAddr = newAddr
		case bad := <-pool.badCh:
			//logger.Info("checkEvent|badCh|%v|%v", bad.id, bad.addr)
			pool.total--
			bad.conn.Close()
		case <-timer.C:
			return
...
func (pool *Pool) checkPool() {
	...
	for pool.total < pool.max {
		...
		if pos >= len(addrs) {
			...
			logger.Error("checkPool|retry_after")
			return
		}
		if conn, err := makeConn(addrs[pos], pool.pwd); err != nil {
			logger.Error("checkPool|makeConn|%v", err)
			pos++
		} else {
			pool.connCh <- conn
			pool.total++
...
func (pool *Pool) testConn() {
	...
	if conn.addr != masterAddr {
		if newconn, err := makeConn(masterAddr, pool.pwd); err != nil {
			logger.Error("bkWorker|makeConn|%v", err)
		} else {
			// 主服务器已经能够连上了
			conn.conn.Close()
			conn = newconn
	...
	if time.Now().Unix() > conn.pingTime {
		...
		if _, err := conn.conn.Do("set", "__ping", "1"); err != nil {
			if strings.HasPrefix(err.Error(), "ERR slavewrite,") {
				...
				//logger.Info("process|ping|%v", err)
				pool.badCh <- conn
			}
		} else {
			logger.Info("bgWorker|ping")
			pool.connCh <- conn
		}
	} else {
		pool.connCh <- conn
...
func (pool *Pool) processSlaveWrite(conn *redisConn, err string) {
	logger.Info("receive|slavewrite")
	// 主从切换了
	if strings.HasSuffix(err, ",unknown") {
		...
		if len(params) != 2 {
			logger.Error("process|slavewrite|format_error|%s", err)
		} else {
			var newip = params[1]
			logger.Info("receive|new_master|%v", newip)
...
func (pool *Pool) processWorker(conn *redisConn, reqs []*redisReq) {
	...
	if err != nil {
		logger.Info("process|processRequest|%v", err)
		if strings.HasPrefix(err.Error(), "ERR slavewrite,") {
			pool.processSlaveWrite(conn, err.Error())
		} else {
			pool.badCh <- conn
